...
springmap = ggmap(myMap) + geom_point(data=all_spring, aes(x=location.long, y=location.lat, col=sp), alpha=0.25) + scale_color_viridis(discrete=TRUE, option="B")
springmap

fallmap = ggmap(myMap) + geom_point(data=all_fall, aes(x=location.long, y=location.lat, col=sp), alpha=0.25) + scale_color_viridis(discrete=TRUE, option="B")
fallmap

breedmap = ggmap(myMap) + geom_point(data=all_breed, aes(x=location.long, y=location.lat, col=sp)) + theme(legend.position ="bottom") + scale_color_viridis(discrete=TRUE, option="B")
breedmap
...
```{r}
ruhu_hull = all_spring[all_spring$sp=="ruhu",]
...
ggmap(myWesternMap) + geom_polygon(data=chulls, aes(x=location.long, y=location.lat, group=yday1, fill=month), col="black", alpha=0.25) + facet_wrap(~year) +
  scale_colour_brewer(palette = "Oranges") + 
  scale_fill_brewer(palette = "Oranges") + ggtitle (sp)

```
Map convex hulls of daily points for fall across years
```{r}
ruhu_hull = all_fall[all_fall$sp=="ruhu",]
...
Make a 2D version of the plots above, but with EVI and t10m as main axies, and colored by elevation
```{r}
